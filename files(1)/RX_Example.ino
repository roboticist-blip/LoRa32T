...
// ─── Pre-shared AES-128 key (must match TX) ──────────────────────────────────
static const uint8_t PSK[16] = {
    0xDE,0xAD,0xBE,0xEF, 0xCA,0xFE,0xBA,0xBE,
    0x01,0x23,0x45,0x67, 0x89,0xAB,0xCD,0xEF
};

// ─── Same layout constants as TX ─────────────────────────────────────────────
#define PAYLOAD_SIZE  64
#define HEADER_SIZE   (1 + 16 + 16 + 4)   // mode + iv + tag + seqNum

// ─── Telemetry mirror struct (must match TX) ──────────────────────────────────
struct Telemetry {
    uint32_t seqNum;
    float    temperature;
    float    pressure;
...
// ─── Sliding window counters (window = 50 packets, paper eq. 4/7/8) ──────────
...
{
    // Subtract outgoing value from total, add incoming
    counter -= win[head];
    win[head] = val;
    counter  += val;
